...
import { AssessmentResult, DOMAIN_COLORS, AppView } from '../types';
  ...
  if (history.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] space-y-6">
        ...
        <h2 className="text-2xl font-bold text-slate-700">尚未有測驗記錄</h2>
        <p className="text-slate-500">完成測驗後，您的結果將會儲存在這裡。</p>
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <h2 className="text-3xl font-bold text-slate-800">歷史記錄</h2>
        <div className="space-x-4">
            <Button variant="outline" onClick={onClear} className="text-red-500 border-red-200 hover:bg-red-50">
                清除記錄
            </Button>
              ...
              <div className="space-y-3">
                {result.topStrengths.slice(0, 3).map((strength, idx) => (
                  <div key={idx} className="flex items-center space-x-2">
                    ...
                    ></span>
                    <span className="text-slate-700 font-medium">{strength.name}</span>
                  </div>
                ))}
                {result.topStrengths.length > 3 && (
                    <div className="text-xs text-slate-400 pl-5">+ 其他 {result.topStrengths.length - 3} 項優勢</div>
                )}
              </div>
            </div>
            <div className="bg-slate-50 p-3 text-center border-t border-slate-100">
                <span className="text-sm text-slate-500">點擊查看詳細分析</span>
            </div>
          </div>
        ))}
      </div>
